...
# agentrt-linux 系统（AirymaxOS System）

[![Version](https://img.shields.io/badge/version-0.1.1-5a6b7e)](https://atomgit.com/openairymax/system)
[![License](https://img.shields.io/badge/license-AGPL--3.0+Apache--2.0-4a90d9)](LICENSE)

> [agentrt-linux（AirymaxOS）](https://atomgit.com/openairymax/agentrt-linux)（智能体操作系统）的系统层子系统。
...
- **DevStation** — 面向 agentrt-linux 贡献者的集成开发工作站镜像

### 与 Airymax commons 的关系

agentrt-linux 系统（AirymaxOS System）复用并扩展了 Airymax 运行时平台的 `commons` 模块。共享配置、打包辅助与 shell 工具在用户态运行时（agentrt）与 OS 级系统层（agentrt-linux）之间共享，确保架构同源、无适配层。
...
- **agentrt-linux 内核（AirymaxOS Kernel）** — 提供系统层打包的内核镜像与模块
- **agentrt-linux 服务态（AirymaxOS Services）** — 提供系统层配置与交付的服务集合
- **Airymax commons** — 提供被复用并扩展的共享配置与打包辅助

### 下游

- **End users** — 通过本子系统产出的 RPM/dnf 面安装 agentrt-linux 的终端用户
- **DevStation users** — 启动 DevStation 镜像参与 agentrt-linux 贡献的开发者
